    ...
    <div v-else class="flex h-full w-full">
      ...
      >
          ...
          <div class="flex items-center justify-between gap-3">
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                ...
                <p class="truncate text-sm font-medium text-slate-800">{{ myUid || 'Connecting...' }}</p>
              </div>
            </div>
            <button
              type="button"
            ...
            <p class="mt-0.5 truncate text-xs text-slate-500">
              {{
                groupPreviewText(group.id) ||
                  (group.id === SYSTEM_GROUP ? '欢迎来到 LINKCONNECT（像 Telegram 一样丝滑）' : '暂无消息')
              }}
            </p>
          </div>
          <span
        ...
        <div class="mt-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
          ...
          <p class="mt-1 leading-5 text-slate-500">
            为了安全，我们只信任首次加载的状态。刷新页面或更换浏览器会被视为可能伪装，
            历史群聊不会自动恢复。尽量不要频繁刷新；如需长期聊天，请把对方加入通讯录。
          </p>
        </div>
        ...
        <button
          type="button"
          @click="openContacts"
          class="mt-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:border-emerald-300 hover:text-emerald-700"
        >
          通讯录
        </button>
      </aside>
      ...
      <main class="relative flex min-w-0 flex-1 flex-col bg-gradient-to-b from-slate-100 via-slate-50 to-slate-100">
        ...
        <div v-if="showMobilePanel" class="absolute inset-0 z-20 md:hidden">
          ...
          ></button>
          <div class="absolute inset-x-2 top-2 max-h-[76dvh] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
           <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
             <div class="min-w-0">
               <p class="text-xs uppercase tracking-wide text-slate-400">Group Panel</p>
               <p class="truncate text-sm font-semibold text-slate-800">{{ myUid || 'Connecting...' }}</p>
             </div>
             <button
                type="button"
                @click="showMobilePanel = false"
           ...
           <div class="max-h-[54dvh] overflow-y-auto p-3">
            ...
            <div class="grid grid-cols-2 gap-2 border-t border-slate-100 p-3">
              ...
              <button
                type="button"
                @click="openContacts"
                class="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700"
              >
                通讯录
              </button>
            </div>
        ...
        >
          ...
          >
            <div class="w-full max-w-xl rounded-3xl border border-slate-200/80 bg-white/85 p-6 text-center shadow-sm backdrop-blur md:p-8">
              <p class="text-xs font-semibold uppercase tracking-[0.2em] text-sky-500">LINKCONNECT</p>
              <h2 class="mt-2 text-2xl font-semibold text-slate-900">欢迎回来</h2>
              <p class="mt-3 text-sm leading-6 text-slate-600">
                这是一个端到端加密聊天室。你可以创建群组、生成邀请文案发到其他社交媒体，或直接粘贴邀请码/邀请链接自动入群。
              </p>
              ...
              <div class="mt-6 grid gap-3 sm:grid-cols-2">
                <button
                  type="button"
                  @click="createGroup"
                  class="inline-flex h-11 items-center justify-center rounded-2xl bg-slate-900 px-4 text-sm font-semibold text-white transition hover:bg-slate-800 active:scale-[0.98]"
                >
                  创建新群组
                </button>
                <button
                  type="button"
                  @click="settingsOpen = true"
                  class="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:border-sky-300 hover:text-sky-700 active:scale-[0.98]"
                >
                  通知与提示音
                </button>
              </div>

              <div class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left">
              ...
              <div class="mt-4 rounded-2xl border border-slate-200 bg-white/90 p-4 text-left">
          ...
          <div v-else class="mx-auto flex w-full max-w-4xl flex-col gap-3">
            <article
              ...
              :class="msg.sender === myUid ? 'justify-end' : 'justify-start'"
            >
              <div class="max-w-[92%] sm:max-w-[75%]">
                <p v-if="msg.sender !== myUid" class="mb-1 px-1 text-[11px] font-medium text-slate-500">
                  {{ msg.sender }}
                </p>
                <div
          ...
          <div class="absolute inset-x-3 top-16 mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div class="max-h-[65dvh] overflow-y-auto px-4 py-4">
              ...
              <div v-else class="mt-3 grid gap-2">
                ...
                >
                  <div class="min-w-0">
                    <p class="truncate text-sm font-semibold text-slate-800">{{ contact.alias }}</p>
                    <p class="truncate text-xs text-slate-500">
                      指纹：{{ contact.fingerprintShort }}
                    </p>
                    <p class="truncate text-xs text-slate-500">
...
const groups = ref([{ id: SYSTEM_GROUP, name: '首页', onlineCount: 0 }]);
...
const joinedGroups = new Set([SYSTEM_GROUP]);
...
const eligibleInviteGroups = computed(() => {
  ...
  return groups.value.filter(
    (g) => g.id && g.id !== active && g.id !== SYSTEM_GROUP && !g.id.startsWith('dm-')
  );
});
...
const addContact = (user) => {
  ...
  ws.send(
    JSON.stringify({
      type: 'contacts_add',
      targetUid: user.uid,
      alias: `用户 ${user.uidShort || user.uid}`,
    })
  );
};
...
const startDirectChat = (user) => {
  ...
  if (!groupId) return;
  ensureGroupInList(groupId, `私聊 · ${user.uidShort}`);
...
const copyInviteLink = async () => {
  ...
  if (!groupId || groupId === SYSTEM_GROUP) {
...
const acceptPairInvite = (msg) => {
  ...
  markPairAccepted(msg.groupId, msg.pairGroupId);
};
...
const ensureIdentitySignKeys = async () => {
  ...
  try {
    const raw = window.localStorage.getItem('telechat.identity.sign');
    if (raw) {
      const jwk = JSON.parse(raw);
      const priv = await importEcdsaPrivateKey(jwk);
      ...
      identitySignKeyPair.value = { privateKey: priv, publicKey: pub };
      identitySignPublicBase64.value = await exportSpkiBase64(pub);
      return;
    }
  } catch {
  ...
  try {
    ...
    window.localStorage.setItem('telechat.identity.sign', JSON.stringify(jwk));
  } catch {
    // no-op
  }
};
...
const ensureIdentityDhKeys = async () => {
  ...
  try {
    const raw = window.localStorage.getItem('telechat.identity.dh');
    if (raw) {
      const jwk = JSON.parse(raw);
      const priv = await importEcdhPrivateKey(jwk);
      const pub = await window.crypto.subtle.importKey(
        'jwk',
        { ...jwk, d: undefined, key_ops: [] },
        { name: 'ECDH', namedCurve: 'P-256' },
        true,
        []
      );
      identityDhKeyPair.value = { privateKey: priv, publicKey: pub };
      identityDhPublicBase64.value = await exportSpkiBase64(pub);
      return;
    }
  } catch {
    // fallback to generate
  ...
  try {
    const jwk = await exportJwk(keyPair.privateKey);
    window.localStorage.setItem('telechat.identity.dh', JSON.stringify(jwk));
  } catch {
    // no-op
  }
};
...
const startPowSolve = async () => {
  ...
  if (!myUid.value || !powState.value.nonce || !powState.value.difficulty) return;
  ...
  try {
    const answer = await solvePow(
      { uid: myUid.value, nonce: powState.value.nonce, difficulty: powState.value.difficulty },
      token
    );
    if (!answer) return;
...
const isDmLocked = (groupId) => {
  const gid = sanitizeGroupId(groupId) || SYSTEM_GROUP;
  return Boolean(dmLocks.value[gid]);
};
...
const sendEncryptedPayload = async (payloadType, payload) => {
  ...
  const recipients = onlineUsers.value.filter(
    (u) => u.uid !== myUid.value && typeof u.publicKey === 'string' && u.publicKey.length > 0
  );
...
const joinGroup = (groupId, inviteCode = '', options = {}) => {
  ...
  if (gid.startsWith('dm-') && !joinedGroups.has(gid)) {
...
const connectWS = () => {
  ...
  ws.onmessage = async (event) => {
    ...
    if (data.type === 'contacts_migrate_done') {
      toast(`通讯录迁移完成（${data.count || 0} 条）。`, 'info');
      migrationCode.value = '';
    ...
    if (data.type === 'pow_verified') {
      ...
      if (pendingDirect.value.groupId && pendingDirect.value.targetUid) {
        ...
        pendingJoin.value = { groupId: '', inviteCode: '', select: true };
      } else if (activeGroup.value !== SYSTEM_GROUP) {
    ...
    if (data.type === 'invite_created') {
      ...
      if (pendingPairInvite.value.reqId && reqId && pendingPairInvite.value.reqId === reqId) {
        ...
        await sendDirectEncryptedPayload(
          'pair',
          {
            kind: 'pair',
            pairGroupId: groupId,
    ...
    if (data.type === 'error') {
      ...
      if (code === 'CONTACT_TARGET_OFFLINE') {
        toast('对方不在线，无法加入通讯录。', 'info');
        return;
      }

      if (code === 'CONTACT_TARGET_UNBOUND') {
        toast('对方设备未绑定，暂不可加入通讯录。', 'info');
        return;
      }
